...
def prepare_visual_states(obs: Dict[str, Dict[str, Any]],
    ...
    if 'dp3_obs' not in obs:
        obs['dp3_obs']= {
            'rgb_obs': {
                'rgb_static': np.stack([rgb_static] * nobs, axis=0),
                'rgb_gripper': np.stack([rgb_gripper] * nobs, axis=0)
            },
            'pcd_obs': {
                'pcd_static': np.stack([static_pcd] * nobs, axis=0),
                'pcd_gripper': np.stack([gripper_pcd] * nobs, axis=0)
            }
        }
    else:
        obs['dp3_obs']['rgb_obs']['rgb_static'] = np.concatenate([
            obs['dp3_obs']['rgb_obs']['rgb_static'][1:], rgb_static[None]
        ], axis=0)
        obs['dp3_obs']['rgb_obs']['rgb_gripper'] = np.concatenate([
            obs['dp3_obs']['rgb_obs']['rgb_gripper'][1:], rgb_gripper[None]
        ], axis=0)
        obs['dp3_obs']['pcd_obs']['pcd_static'] = np.concatenate([
            obs['dp3_obs']['pcd_obs']['pcd_static'][1:], static_pcd[None]
        ], axis=0)
        obs['dp3_obs']['pcd_obs']['pcd_gripper'] = np.concatenate([
            obs['dp3_obs']['pcd_obs']['pcd_gripper'][1:], gripper_pcd[None]
        ], axis=0)
...
def prepare_proprio_states(obs: Dict[str, Dict[str, Any]],
    ...
    if 'proprio' not in obs:
        obs['proprio'] = np.stack([proprio] * nobs, axis=0)
    else:
